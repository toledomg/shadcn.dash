...
export function CalendarMain({ eventDates = [] }: CalendarMainProps) {
          ...
          {calendarDays.map((day) => {
            ...
            return (
              <div
                key={day.toISOString()}
                className={cn(
                  "hover:bg-muted/50 relative min-h-[120px] cursor-pointer border-r border-b p-2 transition-colors last:border-r-0",
                ...
                <div className="space-y-1">
                  {dayEvents.slice(0, 3).map((event) => (
                    <div
                      key={event.id}
                      className={cn(
                        "cursor-pointer truncate rounded px-2 py-1 text-xs text-white transition-opacity hover:opacity-80",
                        event.color
                      )}
                      onClick={(e) => {
                        e.stopPropagation()
  ...
  const renderSidebar = () => (
    <div className="bg-background h-full w-full border-r">
